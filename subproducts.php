<?php include('header.php') ?>
    ...
    <?php $submenu = $_GET['subid'];
      // echo $subcat; 

    
        
        $s2="SELECT * from sub_menu where submenu_id = '$submenu'";
        $sl2=$db->prepare($s2);
        $sl2->execute();
 ...
 while($res3=$sl2->fetch(PDO::FETCH_ASSOC))
 {
    ...
    ?>
			...
			<h1 id="engname" class="entry-title draw-a-line-standby">
				<?php echo $res3["submenu_name"]; ?>	 											
			</h1>
			<h1 id="arabicname" class="entry-title draw-a-line-standby">
				<?php echo $res3["submenu_name_arab"]; ?>	 											
			</h1>
			<div id="engdesc" class="entry-title-subheading"><?php echo $res3["submenu_desc"]; ?></div>
			<div id="arabdesc" class="entry-title-subheading"><?php echo $res3["submenu_desc_arab"]; ?></div>	
			<?php } ?>	
		</div>
			</div>
</div>
</div><div class="container clearfix" style="min-height: 540px">	<div class="page-contents-wrap  ">
    ...
    echo $submenu; 
      ...
      $s2="SELECT * from category where category_submenu = '$submenu' and category_status = 1 ";
        $sl2=$db->prepare($s2);
        $sl2->execute();
 ...
 while($res4=$sl2->fetch(PDO::FETCH_ASSOC))
 {
     ...
     $cat_id = $res4["category_id"];

        $s22="SELECT product_pic from product where product_category = '$cat_id' Limit 1";
        $sl22=$db->prepare($s22);
        $sl22->execute();
        $res42=$sl22->fetch(PDO::FETCH_ASSOC)

    ?>       
       <!--  <?php echo $res42["product_pic"]; ?>   -->     
				...
				<div class="gridblock-background-hover">
					...
					<a class="" href="products.php?subid=<?php echo $res4['category_id'] ?>" >
						...
						<a href="products.php?subid=<?php echo $res4['category_id'] ?>"><img src="admin/uploads/<?php echo $res42["product_pic"]; ?>" style="height: 404px;width: 404px" alt="" class="displayed-image"/></a>
				</div>
					<div class="work-details"><div class="worktype-categories">
						
					</div><h4>

						<a id="engcat" href="products.php?subid=<?php echo $res4['category_id'] ?>"><?php echo $res4["category_name"]; ?></a>
						<!-- <a id="arabcat" href="products.php?subid=<?php echo $res4['category_id'] ?>"><?php echo $res4["category_name_arab"]; ?></a> -->

					</h4><p class="entry-content work-description"></p></div>
				</div>
